...
def _sea_line(sea_temps: list[float] | None) -> str:
    ...
    if values:
        ...
        if round(low) == round(high):
            return f"Море: средняя вода около {_fmt_num(sum(values) / len(values))}°C; лучше утром или ближе к закату."
        return f"Море: средняя вода {_fmt_num(low)}–{_fmt_num(high)}°C; лучше утром или ближе к закату."
    return "Море: данные по воде обновятся ближе к неделе; у берега комфортнее утром и вечером."
...
def _space_line(kp_tuple: tuple[Any, ...] | None) -> tuple[str, bool]:
    ...
    if kp >= 4:
        return f"Kp около {kp:.1f}: неделя без драматизации, но график лучше не перегружать.", True
