...
• Поддержка порывов ветра (wind_gusts_10m) и алиасы ключей для совместимости.
"""
...
import pendulum
from utils import _get  # ожидание: _get(url, **query) -> parsed JSON
...
def _safe_http_get(url: str, **kwargs) -> Optional[Dict[str, Any]]:
    """Вызывает utils._get с timeout (если поддерживается), возвращает dict либо None."""
    try:
        try:
...
def _ensure_aliases_om_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Добавляет дублирующие ключи для совместимости с кодом KLD."""
...
def _openmeteo_hourly_daily(
    ...
    Часовые метки приходят уже в указанной TZ.
    """
...
def day_night_stats(
    ...
    """
    Оценки на «завтра»:
      t_day_max — максимум днём (между рассветом и закатом)
      t_night_min — минимум ночи (0–6)
      rh_* — влажность по всем 24 ч.
    """
    tz_name = tz or "UTC"
