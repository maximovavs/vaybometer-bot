...
ATTEMPTS: List[_AttemptSpec] = [
    _AttemptSpec(hourly=HOURLY_FULL, daily=DAILY_FULL, current_mode="current", current_fields=CURRENT_FULL),
    _AttemptSpec(hourly=HOURLY_NO_TSTORM, daily=DAILY_FULL, current_mode="current", current_fields=CURRENT_FULL),
    _AttemptSpec(hourly=HOURLY_MIN, daily=DAILY_MIN, current_mode="current", current_fields=CURRENT_MIN),
...
def _build_url(
    ...
    if spec.current_mode == "current":
        params["current"] = ",".join(spec.current_fields or CURRENT_MIN)
    else:
...
def _ensure_aliases(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(hourly, dict):
        ...
        if "dewpoint_2m" in hourly and "dew_point_2m" not in hourly:
            hourly["dew_point_2m"] = hourly.get("dewpoint_2m")
    ...
    if isinstance(cur, dict) and "dewpoint_2m" in cur and "dew_point_2m" not in cur:
        cur["dew_point_2m"] = cur.get("dewpoint_2m")
