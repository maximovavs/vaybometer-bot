...
def get_weather(lat: float, lon: float) -> Optional[Dict[str, Any]]:
    """
    Возвращает unified-словарь с прогнозом:
      • для OpenWeather: ключи current, daily, hourly
      • для Open-Meteo: current_weather, daily, hourly
    И в любом случае добавляет поля:
      • strong_wind: bool (скорость > 30 км/ч)
      • fog_alert:   bool (код 45/48)
    ...
    if OWM_KEY:
        for ver in ("3.0", "2.5"):
            url = f"https://api.openweathermap.org/data/{ver}/onecall"
            ow = _get(
                url,
            ...
            if ow and "current" in ow:
                cur = ow["current"]
                # сделаем hourly-оболочку, чтобы интерфейс был единым
                ow["hourly"] = {
    ...
    # 3️⃣ Fallback-эмуляция: только current_weather
    om = _get(
        "https://api.open-meteo.com/v1/forecast",
        latitude=lat, longitude=lon, timezone="UTC",
