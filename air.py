...
• get_kp() — текущий индекс Kp с retry, кешем и проверенными endpoint'ами
"""
...
# Надёжные JSON-источники Kp-индекса
KP_URLS = [
...
def merge_air_sources(src1: Optional[Dict[str,Any]], src2: Optional[Dict[str,Any]]) -> Dict[str, Any]:
    ...
    for k in ("aqi","pm25","pm10"):  # type: ignore
        ...
        base[k] = v1 if v1 not in (None, "н/д") else (src2.get(k) if src2 and src2.get(k) not in (None, "н/д") else base[k])
...
def get_kp() -> Tuple[Optional[float], str]:
    for url in KP_URLS:
        ...
        logging.info("Kp fetch from %s: %r", url, bool(data))
        if not data:
            continue
        try:
    ...
    if kp_cached is not None:
        logging.info("Using cached Kp: %s @%s", kp_cached, ts)
