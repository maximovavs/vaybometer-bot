...
def compute_advice(d: pendulum.Date, phase_str: str) -> List[str]:
    ...
    if gpt:
        prompt = (
            f"Действуй как Авторитетный Астролог, который очень хорошо знает как влияют звезды и луга на жизни людей, и хочет приносить максимальную пользу людям. Пиши только конкретные советы, без лишнишь слов, будто каждое слово это деньги. Дата: {d.to_date_string()}, фаза: {phase_str}. "
