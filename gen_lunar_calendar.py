...
async def gpt_short(date: str, phase: str) -> List[str]:
    ...
    if GPT:
        prompt = (
            f"Дата {date}, фаза {phase}. Действуй как профессиональный астролог, который хорошо знает как звезды и луна влияют на человека, ты очень хочешь помогать людям делать их жизнь лучше, но при этом ты ценишь каждое слово, ты краток будто каждое слово дорого стоит. Дай 3 лаконичных совета, каждый в одной строке, "
...
async def gpt_long(name: str, month: str) -> str:
    ...
    if GPT:
        prompt = (
            f"Месяц {month}. Фаза {name}. Действуй как профессиональный астролог, который хорошо знает как звезды и луна влияют на человека, ты очень хочешь помогать людям делать их жизнь лучше, но при этом ты ценишь каждое слово, ты краток будто каждое слово дорого стоит. Дай 2 коротких предложения, описывающих "
            "энергетику периода. Тон экспертный, вдохновляющий."
        )
        try:
            r = GPT.chat.completions.create(
                    model="gpt-4o-mini",
