...
async def gpt_short(date: str, phase: str) -> List[str]:
    ...
    if GPT:
        prompt = (
            f"Дата {date}, фаза {phase}. Действуй как профессиональный астролог. Дай 3 лаконичных совета, каждый в одной строке, "
...
async def gpt_long(name: str, month: str) -> str:
    ...
    if GPT:
        prompt = (
            f"Месяц {month}. Фаза {name}. Действуй как профессиональный астролог. Дай 2 коротких предложения, описывающих "
