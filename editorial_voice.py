...
CYPRUS_MORNING_VARIANTS = {
    "LOCAL_WEATHER": [
        "погода сегодня локально неоднородная: в разных частях острова условия могут заметно различаться.",
        "день лучше держать гибким: побережье, центр острова и горы могут ощущаться по-разному.",
    ],
    "HOT_UV": [
        # HOT_UV can fire on a UV-only day below the heat threshold, so these phrases
        # describe the load of the day without asserting heat that may not exist.
        "погода сегодня задаёт более требовательный темп, и тело это почувствует.",
        "день может ощущаться тяжелее, чем показывает один термометр.",
        "силы могут уходить быстрее обычного, и усталость к вечеру будет закономерной.",
        "это день про выносливость, а не про скорость.",
    ],
    "WINDY_COAST": [
        "у воды порывы будут заметно менять ощущение комфорта.",
        "ветер сегодня заметно меняет картину дня: ощущения у моря будут меняться от порыва к порыву.",
        "море сегодня живое и шумное — ветер будет ощущаться даже с берега.",
    ],
    "POOR_AIR": [
...
CYPRUS_EVENING_VARIANTS = {
    ...
    "HOT_UV": [
        # Same UV-only caveat as the morning bank: no unconditional heat claims.
        "завтрашний день может ощущаться тяжелее, чем выглядит в прогнозе.",
        "день обещает быть более требовательным к запасу сил, чем кажется утром.",
        "завтра тело может уставать быстрее обычного, и это ожидаемо.",
    ],
    "WINDY_COAST": [
        "у моря завтра ветер будет заметно менять ощущение комфорта.",
        "завтра побережье будет ветреным и живым — порывы заметно повлияют на ощущения у воды.",
