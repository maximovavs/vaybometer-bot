...
Утро: «человечный» обзор СЕГОДНЯ, + 🌇 закат СЕГОДНЯ.
Вечер: единый список городов на ЗАВТРА, + 🌅 рассвет ЗАВТРА.
Астроблок/советы — через gpt.py (логика моделей там).
Kp — как в мировом чате: последний ЗАКРЫТЫЙ 3-часовой интервал NOAA.
"""
...
# ───────────── Kp «как в мировом чате» ─────────────
def _kp_ru_note(kp: float) -> str:
    if kp < 2.0: return "спокойно"
    if kp < 3.0: return "спокойно-умеренно"
    if kp < 4.0: return "умеренно"
    if kp < 5.0: return "активность"
    if kp < 6.0: return "шторм-наблюдение"
    return "буря"

def _kp_world_like() -> Tuple[Optional[float], Optional[str], Optional[int]]:
    """
    Возвращает (kp_value, ru_note, age_minutes), где kp_value — значение ЗАКРЫТОГО 3ч-слота.
    Если NOAA недоступен — вернём None и дадим шанс локальному get_kp().
    """
    try:
        if not requests:
        ...
        for row in data[1:]:
            try:
                ts = pendulum.parse(row[0]).in_tz("UTC")
                val = float(row[1])
                rows.append((ts, val))
            except Exception:
                continue
        if not rows:
            raise RuntimeError("empty NOAA rows")

        now = pendulum.now("UTC")
        # конец текущего слота по 3 часа (00/03/06/…)
        slot_end = now.replace(minute=0, second=0, microsecond=0)
        hours_mod = slot_end.hour % 3
        slot_end = slot_end.subtract(hours=hours_mod)
        closed_end = slot_end  # конец текущего слота
        closed_start = closed_end.subtract(hours=3)

        last_closed = None
        for ts, val in reversed(rows):
            if closed_start <= ts < closed_end:
                last_closed = (ts, val); break
        if not last_closed:
            last_closed = rows[-2] if len(rows) >= 2 else rows[-1]

        kp_val = round(float(last_closed[1]), 1)
        age_min = max(0, int((now - closed_end).total_seconds() // 60))
        return kp_val, _kp_ru_note(kp_val), age_min
    except Exception:
        # фолбэк: старый get_kp()
        try:
            kp_tuple = get_kp() or (None, "н/д", None, "n/d")
            kp = kp_tuple[0] if isinstance(kp_tuple,(list,tuple)) else None
            ...
            if isinstance(ts,int) and ts>0:
                age = int((pendulum.now("UTC").int_timestamp - ts) / 60)
            return (float(kp) if isinstance(kp,(int,float)) else None, str(ks) if isinstance(ks,str) else None, age)
        except Exception:
...
# ---- day temps: today/tomorrow (offset 0/1) ----
def _get_day_temps(lat: float, lon: float, tz: str, day_offset: int) -> Tuple[Optional[float], Optional[float]]:
    try:
        wm = get_weather(lat, lon) or {}
        daily = wm.get("daily") or {}
        times = daily.get("time") or daily.get("date") or []
        ...
        date_obj = pendulum.today(tz).add(days=day_offset)
        idx = None
        for i, t in enumerate(times):
            dti = _parse_iso_to_tz(t, pendulum.timezone(tz))
            if dti and dti.date() == date_obj:
                idx = i; break
        if idx is not None:
            tmax = float(tmaxs[idx]) if idx < len(tmaxs) and tmaxs[idx] is not None else None
            tmin = float(tmins[idx]) if idx < len(tmins) and tmins[idx] is not None else None
            return tmax, tmin
    except Exception:
        pass
    # фолбэк: для завтра есть специальный хелпер; для сегодня — day_night_stats
    if day_offset == 1:
        try:
            tmax, tmin = fetch_tomorrow_temps(lat, lon, tz=tz)
...
# ───────────── «городская» строка ─────────────
def _city_detail_line(city: str, la: float, lo: float, tz_obj: pendulum.Timezone, include_sst: bool, day_offset: int)\
        -> tuple[Optional[float], Optional[str]]:
    tz_name = tz_obj.name
    tmax, tmin = _get_day_temps(la, lo, tz=tz_name, day_offset=day_offset)
    if tmax is None:
    ...
    idx = 0 if day_offset==0 else 1
    wcx = wcx[idx] if isinstance(wcx, list) and len(wcx) > idx else None
    descx = code_desc(wcx) or "—"

    parts = [f"{city}: {tmax:.1f}/{tmin:.1f} °C", f"{descx}"]

    # Вечерний блок (завтра) добавляет ветер/давление/порывы
    if day_offset == 1:
...
def _collect_city_tmax_list(sea_pairs, other_pairs, tz_obj, day_offset: int) -> List[Tuple[str, float]]:
    all_pairs = list(sea_pairs) + list(other_pairs)
    out: List[Tuple[str,float]] = []
    for city, (la, lo) in all_pairs:
        tmax, _ = _city_detail_line(city, la, lo, tz_obj, include_sst=False, day_offset=day_offset)
...
def _llm_morning_intro(fact_text: str, warm: Tuple[str,float], cool: Tuple[str,float], sunset: Optional[str]) -> Optional[str]:
    if not USE_DAILY_LLM or not fact_text or not warm or not cool:
        return None
    try:
        sys = ("Ты — тёплый эксперт-коуч: дружелюбно, кратко, без штампов. "
               "Сделай 1–2 фразы утренного приветствия на русском: упомяни факт дня, "
               "самый тёплый и самый прохладный город (с температурами) и закат (если дан). "
               "Тон заботливый, позитивный, без канцелярита.")
        wrm = f"{warm[0]} ({warm[1]:.0f}°)"
        cl  = f"{cool[0]} ({cool[1]:.0f}°)"
        prompt = (f"Факт дня: {fact_text}\nТёплее всего: {wrm}\nПрохладнее: {cl}\n"
                  f"Закат: {sunset or 'н/д'}\nСделай приветствие.")
        txt = gpt_complete(prompt=prompt, system=sys, temperature=0.3, max_tokens=120)
        line = _sanitize_line((txt or "").splitlines()[0] if txt else "", 220)
        return line or None
    except Exception as e:
        logging.warning("Morning LLM intro failed: %s", e)
        return None
...
def build_message(region_name: str,
    ...
    # защита от перепутанного tz/mode
    if isinstance(tz, str) and tz.strip().lower() in ("morning","evening"):
        logging.warning("build_message: получен tz='%s' (похоже на mode). Перекладываю в mode.", tz)
        mode = tz; tz = "Asia/Nicosia"

    tz_obj = _as_tz(tz)
    mode = (mode or os.getenv("POST_MODE") or os.getenv("MODE") or "evening").lower()
    logging.info("build_message: mode=%s, tz=%s", mode, getattr(tz_obj, "name", tz_obj))

    is_morning = mode.startswith("morn")  # утром — СЕГОДНЯ, вечером — ЗАВТРА
    ...
    # Заголовок
    if is_morning:
        P.append(f"<b>🌅 {region_name}: погода на сегодня ({today.format('DD.MM.YYYY')})</b>")
    else:
        P.append(f"<b>🌅 {region_name}: погода на завтра ({tom.format('DD.MM.YYYY')})</b>")

    wm_region = get_weather(CY_LAT, CY_LON) or {}
    storm_region = storm_flags_for_tomorrow(wm_region, tz_obj)  # нужен на вечер

    # === УТРО (сегодня) ===
    if is_morning:
        rows = _collect_city_tmax_list(sea_pairs, other_pairs, tz_obj, day_offset=0)
        warm = max(rows, key=lambda x: x[1]) if rows else None
        cool = min(rows, key=lambda x: x[1]) if rows else None

        # Факт дня как тон
        fact = get_fact(today, region_name)
        la_sun, lo_sun = _choose_sun_coords(sea_pairs, other_pairs)
        sunset_line = sun_line_for_mode(mode, tz_obj, la_sun, lo_sun)
        sunset_time = sunset_line.split(": ",1)[1] if (sunset_line and ": " in sunset_line) else None

        intro = _llm_morning_intro(fact, warm, cool, sunset_time) if (warm and cool) else None
        if intro:
            P.append(intro)
        else:
            greeting = "👋 Доброе утро!"
            if warm and cool:
                greeting += f" Теплее — {warm[0]} ({warm[1]:.1f}°), прохладнее — {cool[0]} ({cool[1]:.1f}°)."
            P.append(greeting)

        line_all = _format_all_cities_temps_compact(rows)
        if line_all: P.append(line_all)
        if sunset_line: P.append(sunset_line)
        ...
        # Kp: мировая логика
        kp, ks_ru, age_min = _kp_world_like()
        ...
        sw_chunk = (", ".join(parts_sw) + (f" — {wind_status}" if parts_sw and wind_status else "")) if (parts_sw or wind_status) else "н/д"

        if isinstance(kp,(int,float)):
            age_txt = ""
            if isinstance(age_min,int):
                age_txt = f", {age_min//60}ч назад" if age_min >= 180 else (f", {age_min} мин назад" if age_min >= 0 else "")
            P.append(f"🧲 Kp={kp:.1f} ({ks_ru or 'н/д'}{age_txt}) • 🌬️ {sw_chunk}")
            P.append("ℹ️ Kp — глобальный индекс за 3 ч.")
        else:
            P.append(f"🧲 Kp: н/д • 🌬️ {sw_chunk}")

        # Дружеская рекомендация дня
        P.append("💚 Мягкий режим: вода/магний, экраны дозируйте, ранний сон.")
        return "\n".join(P)
    ...
    for city, (la, lo) in list(sea_pairs) + list(other_pairs):
        ...
        tmax, line = _city_detail_line(city, la, lo, tz_obj, include_sst=include_sst, day_offset=1)
    ...
    # Kp мировым способом (для вечернего вывода и рекомендаций)
    kp_val, ks_ru, _ = _kp_world_like()

    P.extend(build_conclusion(kp_val, ks_ru or "н/д", air_now, storm_region, schu_state))
    P.append("———")
