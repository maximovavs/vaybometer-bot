...
## Scope
This repository is the production Cyprus VayboMeter automation. Its sibling regional repository is `maximovavs/vaybometer-bot-kld`.
...
- Never weaken existing safety, provenance, deduplication, weather-consistency, or publication guards merely to make a test pass.
...
- Text/editorial behavior: start with `editorial_voice.py` and the relevant formatter/tests.
- Visual behavior: start with `docs/visual_weather_matrix.md`, then `cyprus_visual_policy.py` and relevant image guards/tests.
- Publication behavior: inspect only the relevant workflow and publication module.
- Weather/AQI/sea/astro/FX/radiation/Schumann issues: open only the corresponding collector/module and targeted tests.
...
Do not assume Cyprus and Kaliningrad implementations are identical. When a task spans both regions, read the sibling repo's own `AGENTS.md` and `PROJECT_STATE.md` and verify parity explicitly.
