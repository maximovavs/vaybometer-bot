...
def _apply_cy_morning_spacing(text: str) -> str:
    """Add visual breathing room to compact Cyprus morning FORMAT_V2 posts."""
    if not _env_on("FORMAT_V2_MORNING_SPACING"):
        return text
    s = str(text or "").strip()
    if "Кипр сегодня" not in s or "🧭 <b>Главный сценарий" in s or "завтра" in s.lower():
        return text
...
def _structure_summary(text: str) -> str:
    ...
    if not ("🌊 <b>Побережье" in s or "🏙 <b>Центр" in s or "🌡" in s):
